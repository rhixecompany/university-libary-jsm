    ...
    <div className="admin-sidebar">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="user">
            <Avatar>
              <AvatarImage src={`${config.env.imagekit.urlEndpoint}/${session?.user?.avatar}`} alt={`avatar ${session?.user?.name}`} />
          ...
          <DropdownMenuLabel className="p-0 font-normal">
            <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
              <Avatar className="size-8 rounded-lg">
                <AvatarImage src={`${config.env.imagekit.urlEndpoint}/${session?.user?.avatar}`} alt={`avatar ${session?.user?.name}`} />
                <AvatarFallback className="rounded-lg">{getInitials(session?.user?.name || 'IN')}</AvatarFallback>
              </Avatar>
